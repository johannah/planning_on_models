...
def make_atari_channel_state(frames, next_frames, device):
    ...
    return states.to(device), next_states.to(device)
...
def forward_pass(model_dict, states, actions, rewards, next_states, index_indexes, phase, info):
    ...
    states, target = make_atari_channel_state(states, next_states, info['device'])
