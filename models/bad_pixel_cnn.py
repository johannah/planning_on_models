...
class GatedMaskedConv2d(nn.Module):
    def __init__(self, mask_type, dim, kernel, residual=True, n_classes=None,
        ...
        if n_classes is not None:
            self.class_cond_embedding = nn.Embedding(n_classes, 2*self.dim)
        ...
        if spatial_condition_size is not None:
            self.spatial_condition_stack = nn.Conv2d(spatial_condition_size, self.dim*2, kernel_size=cond_kernel_shape, stride=1, padding=cond_padding_shape)

        self.vert_stack = nn.Conv2d(self.dim, self.dim*2, kernel_size=vkernel_shape, stride=1, padding=vpadding_shape)
        self.vert_to_horiz = nn.Conv2d(2*self.dim, 2*self.dim, kernel_size=1)
        self.horiz_stack = nn.Conv2d(self.dim, self.dim*2, kernel_size=hkernel_shape, stride=1, padding=hpadding_shape)
        # kernel_size 1 are "fixup layers" to make things match up
        self.horiz_resid = nn.Conv2d(self.dim, self.dim, kernel_size=1, stride=1)
...
class GatedPixelCNN(nn.Module):
    def __init__(self, input_dim=512, dim=256, n_layers=15,
        ...
        self.init_conv = nn.Conv2d(self.input_dim, self.dim, 1)
    ...
    def forward(self, x, class_condition=None, spatial_condition=None, float_condition=None):
        ...
        for i, layer in enumerate(self.layers):
            x_v, x_h = layer(x_v=x_v, x_h=x_h,
                             class_condition=self.class_condition,
                             spatial_condition=self.spatial_condition,
                             float_condition=self.float_condition)
