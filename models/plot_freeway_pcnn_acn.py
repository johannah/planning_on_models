...
def nearest_neighbor_batch():
    while not data_loader.done:
        data, label, data_idx = data_loader.ordered_batch()
        if data.shape[0]:
            z, u_q, s_q = encoder_model(data)
            prior_model.codes[data_idx-largs.number_condition] = u_q.detach().cpu().numpy()
    ...
    test_z, test_u_q, test_s_q = encoder_model(test_data)
