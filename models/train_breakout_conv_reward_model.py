...
def load_avg_grad_cam(rew_model_loadpath, data_buffer, data_buffer_loadpath, DEVICE='cpu'):
    ...
    if os.path.exists(npy_outpath):
        ...
        nancheck = np.where((all_masks.max(axis=-1).max(axis=-1) - all_masks.min(axis=-1).min(axis=-1)) == 0)[0]
        all_masks_normed = (all_masks - all_masks.min(axis=-1).min(axis=-1)[:, None, None]) / ((all_masks.max(axis=-1).max(axis=-1) - all_masks.min(axis=-1).min(axis=-1))[:, None, None])
        all_masks_normed[nancheck, :, :] = 1.
        all_masks = all_masks_normed
