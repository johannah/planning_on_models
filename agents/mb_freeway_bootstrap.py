...
from train_atari_vqvae_diff_action_reward import prepare_state_for_vq, train_vqvae, VQVAErl
...
def pt_latent_learn():# latent_states, actions, rewards, latent_next_states, terminal_flags, masks):
    ...
    # step
    avg_train_losses, next_z_q_x, next_z_e_x, pt_data = train_vqvae(vqvae_model, info, batch)
...
def train_sim(step_number, last_save):
    ...
    while step_number < info['MAX_STEPS']:
        ...
        while epoch_frame < info['EVAL_FREQUENCY']:
            ...
            rewards_weight = 1-np.array(replay_memory.percentages_rewards())
            actions_weight = 1-np.array(replay_memory.percentages_actions())
...
if __name__ == '__main__':
    print("TODODODODODODO - add params from DQN model to the optimizer")
    ...
    info = {
        ...
        "REPLAY_MEMORY_VALID_LOADPATH":"../../model_savedir/MBFreeway_init_valid/MBFreeway_data_0000005001q_train_buffer.npz",
